#!/usr/bin/env python3
"""
Apply positional xGI changes to system files
Handles file locks from running Flask processes
"""
...
def update_system_parameters():
    ...
    backup_path = f"backups/2025-09-23_xgi_implementation/system_parameters.json.pre_update"
    ...
    print(f"✓ Created additional backup: {backup_path}")
    ...
    # Write updated config with retry logic for file locks
    max_retries = 5
    for attempt in range(max_retries):
        try:
            with open(config_path, 'w') as f:
                json.dump(config, f, indent=2)
            print("✓ Successfully updated system_parameters.json")
            break
...
def update_calculation_engine():
    ...
    backup_path = f"backups/2025-09-23_xgi_implementation/calculation_engine_v2.py.pre_update"
...
'''
    ...
    if old_method in content:
        ...
        print("✓ Updated main _calculate_xgi_multiplier method")
    else:
        print("⚠ Could not find exact match for _calculate_xgi_multiplier method")

    # Add new methods before the test section at the end
    insert_point = 'if __name__ == "__main__":'
    if insert_point in content:
        content = content.replace(insert_point, new_methods + '\\n' + insert_point)
        print("✓ Added new positional xGI methods")
    else:
        # Fallback: add at the end of the file
        content += new_methods
        print("✓ Added new positional xGI methods at end of file")

    # Write updated file with retry logic
    max_retries = 5
    for attempt in range(max_retries):
        try:
            with open(engine_path, 'w') as f:
                f.write(content)
            print("✓ Successfully updated calculation_engine_v2.py")
            break
        except (PermissionError, OSError) as e:
            if attempt < max_retries - 1:
                print(f"  Attempt {attempt + 1} failed: {e}. Retrying...")
...
REM Restore files from backup if needed
echo Restoring from backup...
copy "backups\\2025-09-23_xgi_implementation\\system_parameters.json" "config\\system_parameters.json"
copy "backups\\2025-09-23_xgi_implementation\\calculation_engine_v2.py" "calculation_engine_v2.py"
echo Files restored from backup!
pause
'''

    with open("restore_xgi_backup.bat", 'w') as f:
        f.write(restore_script)
    print("✓ Created restore_xgi_backup.bat script")
...
def main():
    ...
    try:
        ...
        print("✓ Created restore script: restore_xgi_backup.bat")
        print()
        print("Next steps:")
        print("1. Restart Flask backend to load new config")
        print("2. Test the new xGI multiplier in dashboard")
        print("3. Update frontend controls for new parameters")

    except Exception as e:
        print(f"\\n✗ ERROR: {e}")
        print("Use restore_xgi_backup.bat to restore if needed")
        raise
